...
class Tasks extends CI_Controller
{
    ...
    {
        if ($this->task_model->mark_completed($task_id)) {
            $project_id = $this->task_model->get_task_project_id($task_id);
            $this->session->set_flashdata('mark_done', 'This task have been completed');
            redirect('projects/display/' . $project_id . '');
        }
    }
    public function mark_incompleted($task_id)
    {
        if ($this->task_model->mark_incompleted($task_id)) {
            $project_id = $this->task_model->get_task_project_id($task_id);
            $this->session->set_flashdata('mark_undone', 'This task have been marked as incompleted');
            redirect('projects/display/' . $project_id . '');
        }
    }
}
